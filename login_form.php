...
$newpasspageLock = 0;
			...
			elseif ( 'resetpass' == $_GET['action'] || 'rp' == $_GET['action'] ){
				...
				if( isset($_POST['pass1'], $_POST['piereg_login_form_nonce']) && wp_verify_nonce( $_POST['piereg_login_form_nonce'], 'piereg_wp_login_form_nonce' ) ){
					$errors = new WP_Error();
	...
	$form_data .= apply_filters("piereg_login_sms_form",$piereg_widget);
		...
		<form method="post" id="piereg_login_form" class="piereg_loginform" name="loginform" action="'.$_SERVER['REQUEST_URI'].'">';
			...
			if( function_exists( 'wp_nonce_field' ))
				$form_data .= wp_nonce_field( 'piereg_wp_login_form_nonce','piereg_login_form_nonce'); 
