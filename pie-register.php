...
//require_once(PIEREG_DIR_NAME.'/dash_widget_stats.php');
	...
	class PieRegister extends PieReg_Base{
		...
		function __construct(){
			...
			*/
			...
			if(is_admin() && ( (isset($_POST['piereg_license_key']) && !empty($_POST['piereg_license_key'])) || $this->piereg_pro_is_activate ) )
			{
				/*if
				(post || has)*/
		...
		*/
		...
		function piereg_register_scripts(){
			...
			wp_register_script('pie_ui_js',plugins_url("/js/pie_ui.js",__FILE__),'jquery','2.0',false);
...
?>
		...
		function checkLogin(){
			...
			if(  isset($_POST['piereg_login_form_nonce']) && wp_verify_nonce( $_POST['piereg_login_form_nonce'], 'piereg_wp_login_form_nonce' )  )
			{
		...
		function PieRegSettingsProcess() {
			...
			if(isset($_POST['piereg_settings_allusers']) && wp_verify_nonce( $_POST['piereg_settings_allusers'], 'piereg_wp_settings_allusers' )) {
				...
				$this->pie_post_array	= $this->pie_post_array	= $this->piereg_sanitize_post_data('piereg_settings_allusers',  ( (isset($_POST) && !empty($_POST))?$_POST : array() ) );
				...
				//echo "<pre>"; print_r($this->pie_post_array); die;
		...
		*/
		...
		function deleteUsers($user_id = 0,$user_email = "",$user_registered = "")
		{
			...
			if( ($grace != 0 and $user_id != 0) and ($user_email != "" and $user_registered != "") and ($register_type != '' and $register_type != "admin_verify") )
			{
			...
			if($this->is_anyone_can_register() || $this->is_pr_preview){
				if(false === $this->is_pr_preview):
					...
					if( function_exists( 'wp_nonce_field' )):
						$registration_from_fields .= wp_nonce_field('piereg_wp_registration_form_nonce','piereg_registration_form_nonce');		
					endif;					 				
				else:
